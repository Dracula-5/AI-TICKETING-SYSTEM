...
from app.db.models import Ticket, User, PriceNegotiation, ProviderCategory
...
async def _notify_ticket_providers(db: Session, ticket: Ticket):
    """
    Alert providers when a new ticket lands. Providers who've registered
    category preferences (via PUT /users/me/categories) only hear about
    tickets in those categories; providers who never configured anything
    are treated as generalists and hear about everything -- so nobody goes
    silently unnotified just because they never touched the settings page.
    """
    category_matches = (
        db.query(User.id)
        .join(ProviderCategory, ProviderCategory.user_id == User.id)
        .filter(
            User.tenant_id == ticket.tenant_id,
            User.role.in_(PROVIDER_ROLES),
            ProviderCategory.category == ticket.category,
        )
        .distinct()
        .all()
    )
    recipient_ids = {r[0] for r in category_matches}

    if not recipient_ids:
        providers_with_prefs = {
            r[0] for r in db.query(ProviderCategory.user_id)
            .join(User, User.id == ProviderCategory.user_id)
            .filter(User.tenant_id == ticket.tenant_id)
            .distinct()
            .all()
        }
        all_providers = (
            db.query(User.id)
            .filter(User.tenant_id == ticket.tenant_id, User.role.in_(PROVIDER_ROLES))
            .all()
        )
        recipient_ids = {r[0] for r in all_providers if r[0] not in providers_with_prefs}

    for recipient_id in recipient_ids:
        await notify(
            db, ticket.tenant_id, recipient_id,
            type_="ticket_created",
            title=f"New {ticket.priority} priority ticket",
            message=f"{ticket.title} ({ticket.category})",
            link=f"/ticket/{ticket.id}",
        )


def _bargaining_counterpart(ticket: Ticket, sender_role: str) -> int | None:
    return ticket.assigned_to_user_id if sender_role == "customer" else ticket.created_by_user_id
...
async def create_ticket(
    ...
    sla_due_time = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(hours=sla_hours)
    ...
    await _notify_ticket_providers(db, new_ticket)
...
async def create_bargaining_offer(
    ticket_id: int,
    ...
    recipient_id = _bargaining_counterpart(ticket, sender_role)
    if recipient_id:
        await notify(
            db, ticket.tenant_id, recipient_id,
            type_="ticket_offer",
            title=f"New offer on '{ticket.title}'",
            message=f"{sender_role.capitalize()} offered Rs {entry.amount}",
            link=f"/ticket/{ticket.id}",
        )
...
async def accept_bargaining_offer(
    ticket_id: int,
    ...
    ticket.price_finalized_at = datetime.now(timezone.utc).replace(tzinfo=None)
    ...
    if recipient_id:
        await notify(
            db, ticket.tenant_id, recipient_id,
            type_="ticket_accepted",
            title=f"Deal finalized on '{ticket.title}'",
            message=f"Accepted at Rs {latest_offer.amount}",
            link=f"/ticket/{ticket.id}",
        )
...
async def reject_bargaining_offer(
    ticket_id: int,
    payload: BargainingActionIn,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    ...
    if recipient_id:
        await notify(
            db, ticket.tenant_id, recipient_id,
            type_="ticket_rejected",
            title=f"Offer rejected on '{ticket.title}'",
            message=f"Rs {latest_offer.amount} was rejected",
            link=f"/ticket/{ticket.id}",
        )
